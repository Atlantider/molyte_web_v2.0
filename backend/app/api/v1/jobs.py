...
def create_md_job(
    ...
    config = {
        ...
        # ECC 配置
        "use_ecc": job_data.use_ecc,
        "ecc_factor": job_data.ecc_factor,
        # Slurm 资源配置
    ...
    try:
        ...
        logger.error(f"Error applying accuracy level parameters: {e}")
        logger.error(traceback.format_exc())
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail=f"Accuracy configuration error: {str(e)}"
        )
...
def batch_create_md_jobs(
    ...
    # Pre-flight check: verify we can submit this many jobs
    total_estimated_hours = len(batch_data.system_ids) * estimate_job_cpu_hours(batch_data)
    ...
    for system_id in batch_data.system_ids:
        try:
            ...
            job_create = MDJobCreate(
                ...
                use_ecc=batch_data.use_ecc,
                ecc_factor=batch_data.ecc_factor
            )
