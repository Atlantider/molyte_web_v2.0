...
ACCURACY_CONFIGS: Dict[AccuracyLevel, Dict[str, Any]] = {
    AccuracyLevel.FAST: {
        ...
        "pressure": 1.0,            # atm
        "freq_trj_npt": 10_000,     # 每 10 ps 输出一次 (10000步)
        "freq_trj_nvt": 5_000,      # 每 5 ps 输出一次 (5000步)
        "thermo_freq": 1_000,       # 每 1 ps 输出一次 (1000步)
    ...
    AccuracyLevel.STANDARD: {
        ...
        "temperature": 298.15,      # K
        "pressure": 1.0,            # atm
        "freq_trj_npt": 100_000,    # 每 100 ps 输出一次 (更稀疏)
        "freq_trj_nvt": 10_000,     # 每 10 ps 输出一次 (10ns生成1000帧)
        "thermo_freq": 5_000,       # 每 5 ps 输出一次
    ...
    AccuracyLevel.ACCURATE: {
        ...
        "temperature": 298.15,      # K
        "pressure": 1.0,            # atm
        "freq_trj_npt": 500_000,    # 每 500 ps 输出一次 (NPT仅需极少帧确认平衡)
        "freq_trj_nvt": 20_000,     # 每 20 ps 输出一次 (20ns生成1000帧，标准分析密度)
        "thermo_freq": 5_000,       # 每 5 ps 输出一次
    ...
    AccuracyLevel.CUSTOM: {
        ...
        "freq_trj_npt": 50_000,     # 参考值：标准模式
        "freq_trj_nvt": 5_000,      # 参考值：标准模式
        "thermo_freq": 1_000,       # 参考值：标准模式
