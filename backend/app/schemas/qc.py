...
class MoleculeType(str, Enum):
    ...
    DIMER = "dimer"  # Dimer 类型（二聚体，Li+与配体）
...
class QCJobCreate(QCJobBase):
    ...
    # QC引擎选择
    qc_engine: Optional[str] = Field(default="gaussian", description="QC计算引擎: gaussian, pyscf, xtb")
    
    # XTB预优化选项（针对cluster和dimer类型自动启用）
    use_xtb_preopt: Optional[bool] = Field(default=False, description="是否使用XTB预优化（cluster/dimer类型建议启用）")
