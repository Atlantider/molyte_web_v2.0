...
"""
    ...
    def _generate_and_run_packmol(self, job_data: Dict, work_dir: Path) -> bool:
        ...
        # 在盒子边界留出 1.0 Å 的 margin，防止原子被放在边界上导致 PPPM 计算出错
        # 这确保了所有原子都在盒子内部，避免 LAMMPS PPPM 计算错误
        margin = 1.0
        box_min = margin
...
"""
    ...
    def _run_moltemplate(self, job_name: str, work_dir: Path) -> bool:
        ...
        # 使用 -nocheck 选项跳过 moltemplate 的语法检查
        # 这是必要的，因为 moltemplate 的检查器在处理嵌套分子定义时有限制
        # 但生成的 .data 和 .in 文件仍然是有效的
        cmd = f"{self.moltemplate_path} -nocheck -pdb {pdb_file} {lt_file}"
        if not self.run_command(cmd):
    ...
    def _parse_lammps_data_for_elements(self, data_file: Path) -> List[str]:
        ...
        try:
            ...
            # 按照 type_id 排序生成 element_list
            element_list = [element_dict[type_id] for type_id in sorted(element_dict.keys())]

            logger.info(f"Parsed {len(element_list)} elements from data file")
            logger.info(f"Element list (by type): {element_list}")
            return element_list
    ...
    def _build_lammps_input_content(self, job_data: Dict, element_list: List[str]) -> str:
        ...
        # 其他变量
        # 温度处理：优先使用 temperature_npt/temperature_nvt，如果没有则使用 temperature，最后默认 298.15K
        default_temp = job_data.get("temperature", 298.15)
        temp_npt = job_data.get("temperature_npt", default_temp)
        temp_nvt = job_data.get("temperature_nvt", default_temp)
...
variable Temp_NPT equal {temp_npt}
variable Temp_NVT equal {temp_nvt}
