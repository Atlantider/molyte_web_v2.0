...
import { ReloadOutlined, DownloadOutlined, BarChartOutlined, PieChartOutlined, SettingOutlined, CheckOutlined } from '@ant-design/icons';
...
export default function RDFCalculatorNature({ jobId }: RDFCalculatorProps) {
  ...
  const pieOption: EChartsOption = {
    ...
    tooltip: {
      ...
      formatter: (params: any) => {
        const titleColor = isDark ? '#E8E8E8' : '#111827';
        return `<div style="font-weight: 600; color: ${titleColor};">${params.name}</div><div style="margin-top: 4px;">CN: <strong>${params.value.toFixed(2)}</strong> (${params.percent.toFixed(1)}%)</div>`;
      },
    },
    legend: {
      orient: 'vertical',
        ...
        <div className="dashboard-card" style={dashboardCardStyle}>
          <div style={{ padding: DASHBOARD_STYLES.cardPadding }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              ...
              <div>
                <div style={{ fontSize: 12, color: token.colorTextSecondary }}>总配位数 @ {cnCutoff.toFixed(1)}Å</div>
                <div style={{ fontSize: 28, fontWeight: 700, color: '#52c41a' }}>
                  {totalCN.toFixed(2)}
                </div>
                <div style={{ fontSize: 11, color: token.colorTextTertiary }}>N(r={cnCutoff.toFixed(1)}Å)</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="dashboard-card" style={dashboardCardStyle}>
          <div style={{ padding: DASHBOARD_STYLES.cardPadding }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              ...
              <div>
                <div style={{ fontSize: 12, color: token.colorTextSecondary }}>平均第一峰位</div>
                ...
                <div style={{ fontSize: 11, color: token.colorTextTertiary }}>Å</div>
              </div>
            </div>
          </div>
        </div>
        ...
        {savedResults.length === 0 ? (
          ...
          <Space wrap size="small">
            {savedResults.map((result, index) => {
              ...
              const isSelected = selectedRdfIds.includes(result.id);
              ...
              return (
                <Tag.CheckableTag
                  ...
                  style={{
                    ...
                    color: isSelected ? 'white' : token.colorText,
                ...
                >
                  <Space size={6}>
                    ...
                    />
                    <Text strong style={{ color: isSelected ? 'white' : token.colorText }}>
                      {result.center_species}
                    </Text>
                    <Text style={{ color: isSelected ? 'white' : token.colorText }}>→</Text>
                    <Text strong style={{ color: isSelected ? 'white' : token.colorText }}>
                      {baseSpecies}
                    </Text>
                    {environment && (
        ...
        <>
                      ...
                      <Col span={14}>
                        <Row gutter={8}>
                          <Col span={8} style={{ textAlign: 'center' }}>
                            <div style={{ fontSize: 10, color: token.colorTextTertiary }}>CN @ {cnCutoff.toFixed(1)}Å</div>
                            <div style={{ fontSize: 14, fontWeight: 700, color: stat.color }}>
                              {stat.cn_at_cutoff.toFixed(2)}
                            </div>
                          </Col>
                          <Col span={8} style={{ textAlign: 'center' }}>
                            <div style={{ fontSize: 10, color: token.colorTextTertiary }}>峰位</div>
                            <div style={{ fontSize: 14, fontWeight: 600, color: token.colorText }}>
                              {stat.first_peak_position?.toFixed(2)} Å
                            </div>
                          </Col>
                          <Col span={8} style={{ textAlign: 'center' }}>
                            <div style={{ fontSize: 10, color: token.colorTextTertiary }}>峰高</div>
                            <div style={{ fontSize: 14, fontWeight: 600, color: token.colorText }}>
          ...
          <Card
            ...
            style={{
              ...
              border: isDark ? '1px solid rgba(24, 144, 255, 0.3)' : '1px solid #bae6fd'
            }}
          >
            <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12 }}>
              <div style={{ width: 4, height: 40, backgroundColor: token.colorPrimary, borderRadius: 2, flexShrink: 0 }} />
              <div>
                <Text strong style={{ fontSize: 13, color: token.colorPrimary }}>分析说明</Text>
                <div style={{ marginTop: 4, fontSize: 12, color: token.colorTextSecondary, lineHeight: 1.6 }}>
