...
export default function Electrolytes() {
  ...
  useEffect(() => {
    ...
    getLabelOptions()
      .then((data) => {
        console.log('Label options loaded:', data);
        setLabelOptions(data);
      })
      .catch((error) => {
        console.error('Failed to load label options:', error);
        // 使用默认选项作为后备 (与后端 electrolyte_labels.py 保持一致)
        setLabelOptions({
          battery_type: {
            label: '电池类型', type: 'single', options: [
              { value: 'lithium_ion', label: '锂离子电池' },
              { value: 'lithium_metal', label: '锂金属电池' },
              { value: 'sodium_ion', label: '钠离子电池' },
              { value: 'potassium_ion', label: '钾离子电池' },
              { value: 'zinc_ion', label: '锌离子电池' },
              { value: 'magnesium_ion', label: '镁离子电池' },
              { value: 'other', label: '其他' },
            ]
          },
          anode_types: {
            label: '负极材料', type: 'multiple', options: [
              { value: 'graphite', label: '石墨' },
              { value: 'silicon', label: '硅/硅碳' },
              { value: 'li_metal', label: '锂金属' },
              { value: 'lto', label: '钛酸锂' },
              { value: 'hard_carbon', label: '硬碳' },
              { value: 'zinc', label: '锌金属' },
            ]
          },
          cathode_types: {
            label: '正极材料', type: 'multiple', options: [
              { value: 'ncm', label: 'NCM三元' },
              { value: 'nca', label: 'NCA' },
              { value: 'lfp', label: '磷酸铁锂' },
              { value: 'lco', label: '钴酸锂' },
              { value: 'lmo', label: '锰酸锂' },
              { value: 'high_nickel', label: '高镍' },
              { value: 'li_rich', label: '富锂锰基' },
              { value: 'sulfur', label: '硫正极' },
              { value: 'air', label: '空气正极' },
            ]
          },
          conditions: {
            label: '工作条件', type: 'multiple', options: [
              { value: 'standard', label: '常规条件' },
              { value: 'high_temp', label: '高温 (>45°C)' },
              { value: 'low_temp', label: '低温 (<0°C)' },
              { value: 'ultra_low_temp', label: '超低温 (<-20°C)' },
              { value: 'wide_temp', label: '宽温域' },
              { value: 'high_voltage', label: '高压 (>4.3V)' },
              { value: 'ultra_high_voltage', label: '超高压 (>4.5V)' },
              { value: 'fast_charging', label: '快充' },
            ]
          },
          electrolyte_type: {
            label: '电解液类型', type: 'single', options: [
              { value: 'organic_liquid', label: '有机液态' },
              { value: 'aqueous', label: '水系' },
              { value: 'ionic_liquid', label: '离子液体' },
              { value: 'high_concentration', label: '高浓度电解液' },
              { value: 'lhce', label: 'LHCE局域化高浓' },
            ]
          },
        });
      });
  ...
  const handleOpenModal = async (electrolyte?: ElectrolyteSystem, isCopy: boolean = false) => {
    if (electrolyte) {
      ...
      try {
        ...
        // 提取配方名称的描述部分（去掉 EL-YYYYMMDD-序号 前缀）
        // 例如：EL-20251208-0001-Li-FSI-DFOB-Sulfolane -> Li-FSI-DFOB-Sulfolane
        ...
        if (prefixMatch) {
          descriptionPart = prefixMatch[2]; // 只保留描述部分
        }

        if (isCopy) {
          displayName = `${editableData.name} (副本)`;
          descriptionPart = `${descriptionPart} (副本)`;
        }

        // 设置表单值
        form.setFieldsValue({
          project_id: editableData.project_id,
          name: descriptionPart, // 只发送描述部分，后端会添加前缀
  ...
  const handleSubmit = async () => {
    try {
      ...
      // 构建新格式数据
      // 注意：name 字段用于用户备注，不是系统生成的配方名称
      const data: any = {
        project_id: values.project_id,
        name: values.name,  // 用户输入的备注信息，后端会保存为 user_note
          ...
          <div>
            <Descriptions bordered column={2} size="small" style={{ marginBottom: 16 }}>
              ...
              {viewingElectrolyte.user_note && (
                <Descriptions.Item label="配方备注" span={2}>{viewingElectrolyte.user_note}</Descriptions.Item>
              )}
