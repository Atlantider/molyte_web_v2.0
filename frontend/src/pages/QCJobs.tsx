...
export default function QCJobs() {
  ...
  const renderStatus = (status: string, record: QCJob) => {
    ...
    // 其他状态的颜色映射
    const color = statusMap[status]?.color || 'default';
  ...
  // 根据精度等级获取基础参数
  const getBaseParams = (level: string) => {
    switch (level) {
      case 'fast':
  ...
  const handleAccuracyChange = (value: string) => {
    ...
    // 获取分子类型，重新应用推荐参数
    // 使用 setTimeout 确保 Select 下拉菜单能正常收回
    setTimeout(() => {
  ...
  const handleCreateOrUpdate = async (values: any) => {
    ...
    try {
      ...
      const jobData: QCJobCreate = {
        ...
        // 自动启用XTB预优化（针对cluster和dimer类型）
        use_xtb_preopt: values.molecule_type === 'cluster' || values.molecule_type === 'dimer',
      } as any;
        ...
        >
            ...
            <Col span={12}>
              <Form.Item name="molecule_type" label="分子类型">
                <Select onChange={handleMoleculeTypeChange}>
                  ...
                  <Select.Option value="dimer">Dimer (二聚体)</Select.Option>
                </Select>
              </Form.Item>
            </Col>
          </Row>
          ...
          <Row gutter={16}>
            <Col span={12}>
              <Form.Item
                ...
                tooltip="PySCF: 开源软件计算引擎; Gaussian: 商业软件(需授权)"
              >
                <Select
                ...
                >
                  {(userProfile?.role === UserRole.ADMIN || userProfile?.can_use_gaussian) && (
                    <Select.Option value="gaussian">
                      <Space>
                        <ThunderboltOutlined style={{ color: '#faad14' }} />
                        <span>Gaussian 16</span>
                        <Tag color="blue">商业软件</Tag>
                      </Space>
                    </Select.Option>
                  )}
          ...
          >
            {({ getFieldValue }) => {
              ...
              // 对于 cluster 和 dimer 类型，SMILES 不是必填项（因为会用XTB预优化）
              const isSpecialType = moleculeType === 'cluster' || moleculeType === 'dimer';
              ...
              return (
                <Form.Item
                  ...
                  extra={isSpecialType ? "Cluster/Dimer 类型会自动使用 XTB 预优化，SMILES 可选" : "可输入自定义SMILES或从下拉列表选择常用分子"}
                >
                  <AutoComplete
              ...
              >
                {accuracyLevels.map(level => (
                  <Select.Option key={level.value} value={level.value}>
                    {level.label} <span style={{ color: token.colorTextSecondary, fontSize: 12 }}>{level.description}</span>
                  </Select.Option>
                ))}
              </Select>
            </Form.Item>
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
              <Space>
                <BulbOutlined style={{ color: token.colorSuccess, fontSize: 18 }} />
