...
const MasterAccountManagement: React.FC = () => {
  ...
  const loadMasterAccounts = async () => {
    ...
    try {
      ...
      if (error.response) {
        ...
        message.error(`加载主账号列表失败: ${error.response.data?.detail || error.response.statusText}`);
      } else if (error.request) {
        console.error('请求错误:', error.request);
        message.error('加载主账号列表失败: 网络错误');
      } else {
        console.error('其他错误:', error.message);
        message.error(`加载主账号列表失败: ${error.message}`);
      }
    } finally {
      setLoading(false);
    }
  ...
  const handleCreateMasterAccount = async (values: any) => {
    try {
      ...
      message.error(error.response?.data?.detail || '创建主账号失败');
    }
  };
  ...
  const handleSubmitEdit = async (values: any) => {
    ...
    try {
      // TODO: 调用更新 API
      message.success('主账号更新成功');
      setEditModalVisible(false);
      editForm.resetFields();
      loadMasterAccounts();
    } catch (error) {
      message.error('更新主账号失败');
    }
  };
    ...
    Modal.confirm({
      ...
      onOk: async () => {
        try {
          ...
          message.success('主账号已删除，已转换为个人账号');
          loadMasterAccounts();
        } catch (error: any) {
  ...
  const handleDeleteSubAccount = async (subAccountId: number) => {
    Modal.confirm({
      ...
      onOk: async () => {
        try {
          if (selectedMaster) {
            await deleteSubAccount(selectedMaster.id, subAccountId);
                    ...
                    Modal.confirm({
                      ...
                      onOk: async () => {
                        try {
                          ...
                          message.success(`已删除 ${selectedRowKeys.length} 个主账号`);
                          setSelectedRowKeys([]);
                          loadMasterAccounts();
                        } catch (error: any) {
